...
public class MagicTextView extends AutoResizeTextView {
    ...
    public void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            if (a.hasValue(R.styleable.MagicTextView_background_)) {
                Drawable background = a.getDrawable(R.styleable.MagicTextView_background_);
                if (background != null) {
                    this.setBackgroundDrawable(background);
                } else {
                    this.setBackgroundColor(a.getColor(R.styleable.MagicTextView_background_, 0xff000000));
                }
            }
    ...
    public void onDraw(Canvas canvas) {
        ...
        //this.setBackgroundWhite(restoreBackground);
        this.setTextColor(restoreColor);
        ...
        paint.setTextSize(item.getTextSize());
        Rect rect = new Rect();
        paint.getTextBounds(text.toString(), 0, text.length(), rect);
        int width = (rect.width() + 20);
        int size = (rect.height()) + 20;

        /*final BgDr backgroundDrawable = new BgDr();
        backgroundDrawable.setShape(GradientDrawable.RECTANGLE);
        backgroundDrawable.setColor(item.getColorBackground());
        backgroundDrawable.setSize(width, size);
        backgroundDrawable.setCornerRadius((float) item.getBackgroundRadiusCorner());
        backgroundDrawable.setBounds(2, 2, 2, 2);
        backgroundDrawable.setAlpha(item.getBackgroundAlpha());*/

        Handler handler = new Handler(Looper.getMainLooper()) {
            @Override
            public void handleMessage(Message msg) {
                // Any UI task, example
                //setBackgroundWhite(backgroundDrawable);
            }
        };
