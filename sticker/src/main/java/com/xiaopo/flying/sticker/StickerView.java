...
public class StickerView extends FrameLayout {
    ...
    private float minScaleWidth;
    private float currentResetAngle = 0f;
    ...
    public StickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        this.sxTMP = 0.0f;
        this.syTMP = 0.0f;
        this.minScaleWidth = 0.0f;
        this.minScaleHeight = 0.0f;
        ...
        try {
            ...
            borderPaint.setAlpha(a.getInteger(R.styleable.StickerView_borderAlpha, 225));
    ...
    public void zoomAndRotateSticker(@Nullable Sticker sticker, @NonNull MotionEvent event) {
        if (sticker != null) {
            ...
            float f = newDistance / this.oldDistance;
            float f2 = newDistance / this.oldDistance;
            matrix.postScale(newDistance / this.oldDistance, newDistance / this.oldDistance, 0.0f, 0.0f);
            float[] fArr = new float[9];
            matrix.getValues(fArr);
            int i = 0;
            float abs = Math.abs(fArr[0] * ((float) this.handlingSticker.getWidth()));
            float abs2 = Math.abs(fArr[4] * ((float) this.handlingSticker.getHeight()));
            if ((abs <= this.minScaleWidth || abs2 <= this.minScaleHeight) && (this.handlingSticker.getCurrentWidth() <=
                    this.minScaleWidth || this.handlingSticker.getCurrentHeight() <= this.minScaleHeight)) {
                int i2 = f > 1.0f ? 1 : 0;
                if (f2 > 1.0f) {
                    i = 1;
                }
                if ((i & i2) == 0) {
                    if (!(this.sxTMP == 0 || this.syTMP == 0)) {
                        this.moveMatrix.postScale(this.sxTMP, this.syTMP, this.midPoint.x, this.midPoint.y);
                    }
                    if (this.isAllowRotate) {
            ...
            this.sxTMP = newDistance / this.oldDistance;
            this.syTMP = newDistance / this.oldDistance;
            this.moveMatrix.postScale(this.sxTMP, this.syTMP, this.midPoint.x, this.midPoint.y);
            if (this.isAllowRotate) {
                this.moveMatrix.postRotate(newRotation - this.oldRotation, this.midPoint.x, this.midPoint.y);
            }
    ...
    public void resetPositionSticker(float rotation) {
        this.currentResetAngle = rotation;
        for (int i = 0; i < stickers.size(); i++) {
            Sticker sticker = stickers.get(i);
            if (sticker != null) {
    ...
    protected void transformSticker(@Nullable Sticker sticker) {
        ...
        matrix.preRotate(-sticker.getCurrentAngle() - this.currentResetAngle, this.midPoint.x, this.midPoint.y);
    ...
    public void resetRotaion() {
        ...
        if (currentSticker != null) {
            ...
            Matrix matrix = this.handlingSticker.getMatrix();
            matrix.preRotate(-currentSticker.getCurrentAngle() - this.currentResetAngle, this.midPoint.x, this.midPoint.y);
    ...
    public float getMinScaleWidth() {
        return this.minScaleWidth;
    }

    public void setMinScaleWidth(float f) {
        this.minScaleWidth = f;
        this.minScaleHeight = f;
    }
